...
class AdvancedSmsClient:
    ...
    def get_ajax_param(
        self, param_name: str, ajax_type: str = "database"
    ) -> Optional[str]:
        """
    ...
    try:
        while True:
            ...
            if status:
                ...
                print(
                    f"Network: {status.network_type.upper()} {'(Connected)' if status.connected else '(Disconnected)'}"
                )
                print(f"Signal: {status.signal_level}/5 bars")
                print(f"Roaming: {'Yes' if status.roaming else 'No'}")
                print(f"SIM: {status.sim_status}")
                print(
                    f"Battery: {status.battery_level}% {'(Charging)' if status.battery_charging else ''}"
                )
