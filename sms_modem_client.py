...
class SmsModemClient:
    ...
    def acknowledge_sms_notifications(
        self, notifications: List[SmsNotification]
    ) -> bool:
        """
    ...
    def poll_sms(
        self, callback, auto_acknowledge: bool = True, interval: Optional[float] = None
    ) -> None:
        """
            ...
            while True:
                ...
                if notifications:
                    ...
                    if send_confirmations:
                        logger.info(
                            f"Sent {len(send_confirmations)} message(s) successfully"
                        )
